...
if [ ${DYNAMIC} = "False" ]; then
    trtexec --onnx=$MODEL --saveEngine=${OUTPUT_PATH}_fp32.trt 
    trtexec --onnx=$MODEL --saveEngine=${OUTPUT_PATH}_fp16.trt --fp16 

    if [ ${IO16} = "True" ]; then
        trtexec --onnx=$MODEL --saveEngine=${OUTPUT_PATH}_fp32_io16.trt --inputIOFormats=fp16:chw --outputIOFormats=fp16:chw 
        trtexec --onnx=$MODEL --saveEngine=${OUTPUT_PATH}_fp16_io16.trt --inputIOFormats=fp16:chw --outputIOFormats=fp16:chw --fp16
    fi
else
    trtexec --onnx=$MODEL --saveEngine=${OUTPUT_PATH}_fp32.trt --minShapes=$MIN_SHAPE --optShapes=$OPT_SHAPE --maxShapes=$MAX_SHAPE
    trtexec --onnx=$MODEL --saveEngine=${OUTPUT_PATH}_fp16.trt --minShapes=$MIN_SHAPE --optShapes=$OPT_SHAPE --maxShapes=$MAX_SHAPE --fp16
